...
interface PayoutModalProps {
  ...
  onClose: () => void;
  balance: number;
  onSubmit: (amount: number, method: string) => Promise<PayoutData>;
}

export const PayoutModal = ({ isOpen, onClose, balance, onSubmit }: PayoutModalProps) => {
  const { showBalance } = useBalanceStore();
  const [amountInput, setAmountInput] = useState<string>("");
  const [methodInput, setMethodInput] = useState<string>("MTN Mobile Money");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [successDetails, setSuccessDetails] = useState<PayoutData | null>(null);

  // Sync default amount value when balance changes or modal opens
  useEffect(() => {
    if (isOpen) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await onSubmit(numericAmount, methodInput);
              ...
              <div className="text-center py-6 space-y-4 font-sans">
                ...
                <div className="rounded-xl border border-border/80 bg-secondary/10 p-4 space-y-2.5 text-xs text-left max-w-sm mx-auto">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-foreground">{successDetails.method}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-emerald-500 font-bold">2 - 5 Hours</span>
                  </div>
                </div>

                <p className="text-[10px] text-muted-foreground/80 leading-relaxed max-w-xs mx-auto">
                  A confirmation email has been dispatched. Funds will clear into your Mobile Money wallet shortly.
                </p>
              ...
              <form onSubmit={handleSubmit} className="space-y-5 font-sans">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Withdraw funds from your EasyBeats available studio balance.
                  </p>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="space-y-2">
                    {[
                      { id: "MTN Mobile Money", name: "MTN Mobile Money (**** 2981)", details: "Settles in 2-5 Hours • Standard MM rates" },
                      { id: "Bank Transfer", name: "Barclays Bank (**** 4812)", details: "Settles in 1-2 Biz Days • $1.50 processing fee" },
                      { id: "PayPal", name: "PayPal (paypal@cloudnine.com)", details: "Settles in 24 Hours • Instant confirmation" },
                    ].map((option) => (
                      <button
                        key={option.id}
                        type="button"
                        onClick={() => setMethodInput(option.id)}
                        className={cn(
                          "flex w-full flex-col rounded-xl border p-3 text-left transition-all",
                          methodInput === option.id
                      ...
                      >
                        <span className="text-xs font-bold text-foreground leading-none">{option.name}</span>
                        <span className="text-[9px] text-muted-foreground/80 mt-1">{option.details}</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex gap-2 p-3 rounded-xl bg-secondary/20 border border-border/80 text-[10px] text-muted-foreground leading-relaxed">
                  ...
                  <span>Please ensure your payout credentials are valid. Failed MM payouts trigger security locks.</span>
                </div>

                {/* Submit Button */}
                <button
                  type="submit"
                  disabled={isSubmitting || parseFloat(amountInput) > balance || parseFloat(amountInput) < 50}
